...
C     ARRAYS FOR THE POLAR DECOMPOSITION     
     ...
     & xtemp(3,3), xS(3,3), Xparams(11), xSTRESS(3,3)
...
C ----------------------------------------------------------------
	    ...
	    xb   = PROPS(6)
		xTNF = PROPS(7)   ! TNF PCR Expression
		write(6,*)
	    xTGF = PROPS(8)    ! TGF PCR Expression
	    write(6,*)
		xLeu  = PROPS(9)    ! IL6 PCR Expression
        write(6,*)
...
! 	  Use stress from previous increment
	  ...
	  Xparams(5) = xTNF   ! TNF PCR Expression
	  Xparams(6) = xTGF   ! TGF PCR Expression
	  Xparams(7) = xLeu    ! IL6 PCR Expression
	  write(6,*) xLeu
	  !Xparams(8) = a1    ! Factoring term for TNF-a
	  !Xparams(9) = b1    ! Factoring term for TGF-B
	  !Xparams(10) = c1    ! Factoring term for IL-6
...
C-----------------------------------------------------------------------------
      ...
      intent(out) dydx
      REAL x, y, dydx
      DIMENSION Xparams(11), y(1), dydx(1)
...
C 	  Growth Parameters (only needed in SUBROUTINE)
	  ...
	  xTNF = Xparams(5)   ! TNF-α PCR Expression
	  xTGF = Xparams(6)    ! TGF-β PCR Expression
	  xLeu  = Xparams(7)    ! IL-6 PCR Expression
      xtrS = Xparams(1) ! trace of stress
      ct   = y(1) ! number of cells
	  xa1 = -0.075 ! TNF-α Signaling Constant
	  xb1 = 0.04  ! TGF-β Signaling Constant
	  xc1 = 0.125 ! IL-6 Signaling Constant 
	  
      IF (xtrS.LE.0.0) THEN
		dgdt = (ct/xtau)*((1.0d0 + (xtrS/(3.0*p0)) - xb) + (xa1*xTNF) + (xb1*xTGF) + (xc1*xLeu))
      ELSE
		dgdt = (ct/xtau)*(1.0d0 - xb + (xa1*xTNF) + (xb1*xTGF) + (xc1*xLeu))
      ENDIF
C 	  
      dydx(1) = dgdt
